...
in
{
  options.mymodules.sway.enable = lib.mkOption {
    default = false;
    example = true;
  };

  config = lib.mkIf config.mymodules.sway.enable {
    ...
    home-manager.users.jj = { pkgs, ... }: {
      ...
      xdg.configFile."waybar/config".source = ./waybar-theme/config;
      xdg.configFile."waybar/style.css".source = ./waybar-theme/style.css;
          ...
          bars = [
            {
              ...
              fonts = { names = [ "Fira Code" ]; size = 9.0; style = "Normal"; };
              position = "top";
              # FIXME: i3status-rust fails to build
